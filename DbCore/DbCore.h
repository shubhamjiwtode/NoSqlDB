...
// ver 1.2                                                         //
// SHUBHAM JIWTODE,                  source-Jim Fawcett            //
...
namespace NoSqlDb
{
  ...
  public:
	...
	enum CheckinStatus { Open, Close, Pending };

	void CheckinStat(CheckinStatus stat) { Checkin_ = stat; }
	CheckinStatus CheckinStat() const { return Checkin_ ; }

  private:
	CheckinStatus Checkin_;
    std::string name_;
    std::string descrip_;
    DateTime dateTime_;
    Children children_;
  ...
  */
  inline void showHeader(bool showKey = true, std::ostream& out = std::cout)
  {
    ...
    out << std::setw(28) << std::left << "Payload";
  ...
  {
	  //std::string status = (el.CheckinStat()) ? "Open" : "Close";
    out << "\n  ";
    out << std::setw(15) << std::left << el.name().substr(0, 13);
    out << std::setw(26) << std::left << std::string(el.dateTime());
    out << std::setw(30) << std::left << el.descrip().substr(0, 18);
    out << std::setw(28) << std::left << std::string(el.payLoad()).substr(0, 28);
  ...
  void showRecord(const Key& key, const DbElement<P>& el, std::ostream& out = std::cout)
  {
    out << "\n  ";
	std::string status;
	if (el.CheckinStat()==DbElement<PayLoad>::Open) { status="Open"; }
	else if ((el.CheckinStat() == DbElement<PayLoad>::Pending))
	{	status = "Pending";}
	else { status = "Close"; }
    out << std::setw(30) << std::left << key.substr(0, 25);
    out << std::setw(15) << std::left << el.name().substr(0, 13);
    out << std::setw(26) << std::left << std::string(el.dateTime());
    out << std::setw(30) << std::left << el.descrip().substr(0, 25);
    out << std::setw(28) << std::left << std::string(el.payLoad()).substr(0, 18);
	out << std::setw(10) << std::left << status;
	Children categories = el.payLoad().categories();
    Children children = el.children();
