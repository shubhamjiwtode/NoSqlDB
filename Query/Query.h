...
namespace NoSqlDb
{
  ...
  public:
    ...
    bool matchChildren();
	void category(std::vector<std::string> cats) { cats_ = cats; }
	bool matchCategory();
    ///////////////////////////////////////////////////////////////
  ...
  bool Conditions<P>::match()
  {
    try
    {
      return matchName() && matchDescription() && matchChildren() && matchDateTimeInterval()&& matchCategory();
    }
  ...
  template<typename P>
  inline bool Conditions<P>::matchCategory()
  {
	  ...
	  P p;
	  p = pDbElem_->payLoad();
	 Keys::iterator start = p.categories().begin();
	 Keys::iterator end = p.categories().end();
	  for (Key key : cats_)
	  {
		  if (std::find(start, end, key) == end)
			  return false;
  ...
  */
  ...
  template<typename P>
  inline void Query<P>::getWOParent()
  {
	  newkeys.clear();
	  DbElement<P> elem;
	  ...
	  for (auto key:db_.keys())
	  {
		  ...
		  for (auto item:db_)
		  {
			  ...
			  if (std::find(childs.begin(), childs.end(), key) == childs.end())
			  {
				  i++;
			  }
